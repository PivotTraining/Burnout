  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="text-navy/35 leading-none" style={{ fontSize: size * 0.1 }}>
          burnout risk
        </span>
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-border-gray p-6 text-center">
        ...
        <span
          ...
          style={{ backgroundColor: colorOf(composite.band) }}
        >
          {composite.band} risk
        </span>
      </div>
      ...
      {topDriver && (
        <div className="bg-navy rounded-2xl p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-white/35 text-xs font-bold uppercase tracking-wider mb-1">
                Your top driver
              </p>
              <p className="text-white font-bold text-base mb-1">
                {SUBSCALE_LABELS[topDriver]} — {results.subscales[topDriver].pct}%
              </p>
              <p className="text-white/55 text-sm leading-relaxed">
                This is the workplace driver feeding your burnout the hardest right now.
                Targeted action here will move the composite faster than working on
                symptoms alone.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-border-gray p-6">
        ...
        <SymptomBars subscales={results.subscales} />
      </div>

      {/* Drivers */}
      <div className="bg-white rounded-2xl border border-border-gray p-6">
        <p className="text-xs font-bold uppercase tracking-widest text-navy/30 mb-4">
          Workplace drivers (Areas of Worklife)
        </p>
        <DriversChart
      ...
      {Object.values(openResponses).some((v) => v && v.trim().length > 0) && (
        <div className="bg-white rounded-2xl border border-border-gray p-6">
          <p className="text-xs font-bold uppercase tracking-widest text-navy/30 mb-4">
            What you said in your own words
          </p>
          <div className="space-y-4">
            {OPEN_ENDED.map((p) => {
              ...
              return (
                <div key={p.id}>
                  ...
                  <blockquote className="text-sm text-navy/80 italic leading-relaxed border-l-2 border-ember/40 pl-3">
                    {v.trim()}
                  </blockquote>
                </div>
              );
